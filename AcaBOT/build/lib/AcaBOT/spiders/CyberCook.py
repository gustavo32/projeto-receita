...
class CybercookSpider(scrapy.Spider):
    ...
    complement = ['saladas', 'massas', 'bebidas', 'lanches', 'legumes', 'peixes-e-frutos-do-mar',
                  ...
                  'risotos', 'bolos', 'acompalhamentos']        #definindo os complementos da URL padrão a serem acessadas
    incremento = 0
    pattern_url = 'https://cybercook.uol.com.br/receitas/{}'    #Definindo a URL padrão
    start_urls = [pattern_url.format(complement[incremento])]   #Iniciando o BOT na seção de saladas do site
    ...
    def parse(self, response):                                  #Função inicial de scrapping
        ...
        for url in response.css('div > section > div > div > h3 > a::attr(href)').extract(): #Seletor CSS para pegar os links das receitas (40 por página)
        ...
        else:                                                         #Se não houver próxima página, siginifica que essa categoria acabou, então:

            self.incremento += 1                                      #dessa forma, a próxima categoria de pratos do sites é requisitada, e o processo se repete
            yield scrapy.Request(url=self.pattern_url.format(self.complement[self.incremento]), callback=self.parse)
    ...
    def parse_details(self, response):                         #Essa é a função feita específicamente para coletar dados das páginas contendo as receitas
        ...
        data['categoria'] = self.complement[self.incremento].replace("-", " ")      #Aqui é atribuido a qual categoria esse receita pertence
