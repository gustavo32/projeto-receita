...
class AperitivosEantepastosSpider(scrapy.Spider):
    name = 'aperitivos-e-antepastosBOT'

    allowed_domains = ['cybercook.uol.com.br']                  #Definindo o domínio de busca
    start_urls = ['https://cybercook.uol.com.br/receitas/aperitivos-e-antepastos']
    ...
    def parse_details(self, response):                         #Essa é a função feita específicamente para coletar dados das páginas contendo as receitas
        ...
        data['categoria'] = "aperitivos-e-antepastos"                               #Aqui é atribuido a qual categoria esse receita pertence
        ...
        for p in ptag:
            if re.search(find_autor, p):
                data['autor'] = re.findall(find_autor, p)[0]
            elif re.search(find_tempo, p):
                data['tempo_de_preparo'] = re.findall(find_tempo, p)[0]
            elif re.search(find_rendi, p):
                data['porcoes'] = int(re.findall(find_rendi, p)[0])

        comentario = response.css('#cc-container > div > div > div >div.grey--dark.p15.mb10.ln30').extract()

        if comentario:

            comentario = comentario[0]

            if re.search(find_tags, comentario):
                for tag in re.findall(find_tags, comentario):
                    tag = "<" + tag + ">"
                    comentario = comentario.replace(tag, "")
